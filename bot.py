import time
import os
import requests
import pandas as pd
import sys
import logging
from datetime import datetime, timezone

from strategy import get_trend_signal
from iqoptionapi.stable_api import IQ_Option

logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")

EMAIL = os.getenv("IQ_EMAIL")
PASSWORD = os.getenv("IQ_PASSWORD")
TOKEN = os.getenv("TELEGRAM_TOKEN")
CHAT_ID = os.getenv("TELEGRAM_CHAT_ID")

EXPIRATION = 1
BASE_AMOUNT = 25
TIMEFRAME_M1 = 60

PAIRS = [
    "EURUSD-OTC", "GBPUSD-OTC", "USDCHF-OTC",
    "EURGBP-OTC", "EURJPY-OTC", "GBPJPY-OTC"
]

MAX_DAILY_TRADES = 8
MAX_LOSS_STREAK = 2
PAUSE_TIME = 1800
FUERZA_MINIMA = 55

DAILY_TRADES = 0
CURRENT_DAY = datetime.now(timezone.utc).day
LOSS_STREAK = 0
LAST_LOSS = 0
LAST_TRADE = None

def send(msg):
    if TOKEN and CHAT_ID:
        try:
            requests.post(f"https://api.telegram.org/bot{TOKEN}/sendMessage",
                          data={"chat_id": CHAT_ID, "text": msg, "parse_mode": "HTML"}, timeout=8)
        except Exception as e:
            logging.error(f"Telegram: {str(e)}")

def es_hora_optima():
    hora = datetime.now(timezone.utc).hour-5
    return 2 <= hora <= 12

def reset_day():
    global DAILY_TRADES, CURRENT_DAY, LOSS_STREAK, LAST_TRADE
    today = datetime.now(timezone.utc).day
    if today != CURRENT_DAY:
        DAILY_TRADES = 0
        LOSS_STREAK = 0
        LAST_TRADE = None
        send("🔄 Nuevo día iniciado | Estrategia de puntos de alta probabilidad activa")

def connect():
    attempts = 0
    while attempts < 5:
        try:
            iq = IQ_Option(EMAIL, PASSWORD)
            status, _ = iq.connect()
            if status:
                iq.change_balance("PRACTICE")
                send(f"✅ Conectado | Saldo: ${iq.get_balance():.2f}")
                return iq
        except Exception as e:
            send(f"❌ Error conexión: {str(e)}")
        attempts += 1
        time.sleep(5)
    time.sleep(30)
    return connect()

def get_df(iq, pair):
    try:
        if not iq.check_connect():
            iq = connect()
        data = iq.get_candles(pair, TIMEFRAME_M1, 50, time.time())
        if not data or len(data) < 25:
            return None
        df = pd.DataFrame(data)
        df.rename(columns={"max": "high", "min": "low"}, inplace=True)
        df[["open", "close", "high", "low", "volume"]] = df[["open", "close", "high", "low", "volume"]].astype(float)
        return df
    except Exception as e:
        logging.error(f"Datos {pair}: {str(e)}")
        return None

def main():
    global LOSS_STREAK, LAST_LOSS, DAILY_TRADES, LAST_TRADE
    iq = connect()
    last_candle = None

    while True:
        try:
            reset_day()
            if not es_hora_optima():
                time.sleep(60)
                continue
            if not iq.check_connect():
                iq = connect()
                time.sleep(2)
                continue
            if DAILY_TRADES >= MAX_DAILY_TRADES:
                time.sleep(60)
                continue
            if LOSS_STREAK >= MAX_LOSS_STREAK:
                remaining = int(PAUSE_TIME-(time.time()-LAST_LOSS))
                if remaining > 0:
                    time.sleep(10)
                    continue
                else:
                    LOSS_STREAK = 0
                    LAST_TRADE = None
                    send("✅ Reanudando búsqueda de puntos de entrada")

            server_time = iq.get_server_timestamp()
            sec = server_time % 60
            current_candle = int(server_time // 60)

            if current_candle == last_candle:
                time.sleep(0.1)
                continue
            last_candle = current_candle

            mejor_opcion = None
            mayor_fuerza = 0

            if 35 <= sec <= 55:
                for pair in PAIRS:
                    df = get_df(iq, pair)
                    if df is None:
                        continue
                    resultado = get_trend_signal(df)
                    if resultado:
                        signal, fuerza, direccion = resultado
                        if fuerza >= FUERZA_MINIMA and fuerza > mayor_fuerza:
                            mayor_fuerza = fuerza
                            mejor_opcion = (pair, signal, fuerza, direccion)

            if 57 <= sec <= 59.9 and mejor_opcion:
                pair, signal, fuerza, direccion = mejor_opcion
                if (pair, signal) == LAST_TRADE:
                    continue
                LAST_TRADE = (pair, signal)

                send(f"""🎯 <b>PUNTO DE ALTA PROBABILIDAD</b>
💹 Activo: {pair}
📈 Tendencia: {direccion.upper()}
💪 Fuerza: {fuerza}/100
📊 Señal: {'🟢 COMPRA' if signal == 'call' else '🔴 VENTA'}
⚡ Ejecutando...""")

                status, trade_id = iq.buy(BASE_AMOUNT, pair, signal, EXPIRATION)
                if status:
                    DAILY_TRADES += 1
                    send(f"🚀 Operación abierta | Monto: ${BASE_AMOUNT:.2f} | {DAILY_TRADES}/{MAX_DAILY_TRADES}")
                    time.sleep(65)
                    res = iq.check_win_v4(trade_id)
                    if res is None:
                        send("⚠️ Sin resultado disponible")
                        continue
                    if res < 0:
                        LOSS_STREAK += 1
                        LAST_LOSS = time.time()
                        send(f"❌ LOSS | -${abs(res):.2f} | Rachas: {LOSS_STREAK}/{MAX_LOSS_STREAK}")
                    else:
                        LOSS_STREAK = 0
                        send(f"✅ WIN | +${res:.2f}\n_________________________")
                else:
                    send("❌ No se pudo ejecutar la operación")

            time.sleep(0.05)

        except Exception as e:
            send(f"💥 Error: {str(e)} | Reiniciando...")
            time.sleep(5)
            iq = connect()

if __name__ == "__main__":
    required = ["IQ_EMAIL", "IQ_PASSWORD", "TELEGRAM_TOKEN", "TELEGRAM_CHAT_ID"]
    missing = [var for var in required if not os.getenv(var)]
    if missing:
        print(f"❌ Faltan variables: {', '.join(missing)}")
        sys.exit(1)
    main()
